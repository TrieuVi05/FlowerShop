...
<style>
    ...
    .bg-gradient-danger  { background: linear-gradient(45deg, #e74a3b 0%, #be2617 100%); }
    ...
    .summary-label { color: #858796; font-size: 0.9rem; }
    .summary-value { font-weight: bold; color: #5a5c69; }
</style>
      ...
      <div class="d-sm-flex align-items-center justify-content-between mb-4">
         ...
         <div class="d-none d-sm-inline-block">
             <div class="btn-group shadow-sm" role="group">
                 <a href="index.php?type=today&from_date=<?=date("Y-m-d")?>&to_date=<?=date("Y-m-d")?>" class="btn btn-sm btn-white text-primary border <?=$type=="today"?"active bg-primary text-white":""?>">Hôm nay</a>
                 <a href="index.php?type=yesterday&from_date=<?=date("Y-m-d",strtotime("-1 days"))?>&to_date=<?=date("Y-m-d", strtotime("-1 days"))?>" class="btn btn-sm btn-white text-primary border <?=$type=="yesterday"?"active bg-primary text-white":""?>">Hôm qua</a>
                 <a href="index.php?type=thisweek&from_date=<?=date("Y-m-d",strtotime("this week"))?>&to_date=<?=date("Y-m-d")?>" class="btn btn-sm btn-white text-primary border <?=$type=="thisweek"?"active bg-primary text-white":""?>">Tuần này</a>
                 <a href="index.php?type=thismonth&from_date=<?=date("Y-m-d",strtotime("this month"))?>&to_date=<?=date("Y-m-d")?>" class="btn btn-sm btn-white text-primary border <?=$type=="thismonth"?"active bg-primary text-white":""?>">Tháng này</a>
             </div>
             <div class="dropdown d-inline-block ml-2">
                <button class="btn btn-sm btn-primary shadow-sm dropdown-toggle" type="button" data-toggle="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in p-3" style="width:300px">
                    <form action="index.php">
                        <div class="form-group mb-2">
                            ...
                            <input type="date" class="form-control form-control-sm" name="from_date" required value="<?=$type == "custom" ? $_GET["from_date"]: ""?>">
                        </div>
                        <div class="form-group mb-2">
                            <small class="text-muted">Đến ngày:</small>
                            <input type="date" class="form-control form-control-sm" name="to_date" required value="<?=$type == "custom" ? $_GET["to_date"]: ""?>">
                        </div>
            ...
            <div class="card-dashboard bg-gradient-danger text-white h-100 py-2">
               <div class="card-body">
                  <div class="card-content">
                      ...
                      <div class="h5"><?= $cancel_number ?></div>
                  </div>
               ...
               <div class="card-body">
                  ...
                  <div class="mt-4 pt-3 border-top">
                      ...
                      <div class="progress mb-1" style="height: 10px;">
                          ...
                          <div class="progress-bar bg-danger" role="progressbar" style="width: <?=100-$rate?>%"></div>
                      </div>
    ...
    if (ctx) {
        var myChart = new Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    xAxes: [{ gridLines: { display: false, drawBorder: false }, barPercentage: 0.5 }],
                    yAxes: [{ ticks: { beginAtZero: true, stepSize: 1, padding: 10 }, gridLines: { color: "rgb(234, 236, 244)", borderDash: [2], drawBorder: false } }]
                },
